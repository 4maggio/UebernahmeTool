...
async function importKnowledge() {
    ...
    for (const filePath of files) {
        try {
            ...
            // Normalize nested title/summary/content (YAML may use title.de or title_de)
            const title_de = entry.title_de || (entry.title && entry.title.de) || null;
            const title_en = entry.title_en || (entry.title && entry.title.en) || title_de;
            const content_de = entry.content_de || (entry.content && entry.content.de) || '';
            const content_en = entry.content_en || (entry.content && entry.content.en) || content_de;
            const summary_de = entry.summary_de || (entry.summary && entry.summary.de) || null;
            const summary_en = entry.summary_en || (entry.summary && entry.summary.en) || summary_de;

            if (!entry.slug || !entry.category || !title_de) {
                console.warn(`  [skip] ${filePath} — missing required fields (slug, category, title)`);
                continue;
            }
